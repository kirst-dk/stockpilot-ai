...
- **On-Chain Transparency** — Agent actions (buy/sell/rebalance) recorded on Mantle smart contracts; **ERC-8004 agent identity** registered on the canonical IdentityRegistry as [**agentId 143**](https://8004scan.io/agents/mantle/143) (indexed & verified on 8004scan), plus on-chain reputation, verifiable-AI rationale anchoring, and **on-chain compliance attestation** (`verifyCompliance(report)` → `MATCH`)
...
| StockPilotComplianceAttestor (on-chain compliance) | Mantle Mainnet | [`0x6d8aADb868CF8d2C7031d593D78b11119D0f3e72`](https://mantlescan.xyz/address/0x6d8aADb868CF8d2C7031d593D78b11119D0f3e72#code) |
...
- **AI-assisted compliance gate (live).** Before any buy, the agent runs a pre-trade compliance check: (1) asset eligibility flag (is this xStock/USDY restricted in the declared region?), (2) sanctioned-address screening of the wallet, (3) per-asset risk disclosure shown in the confirm modal. Legs that touch a restricted asset are blocked and surfaced honestly (their USDC is kept, not silently traded).
- **On-chain compliance attestation (verifiable, not a claim).** Every autonomous cycle commits a `keccak256` hash of its full compliance verdict (region gate + wallet screening + per-layer eligibility + blocked legs) to the [`StockPilotComplianceAttestor`](https://mantlescan.xyz/address/0x6d8aADb868CF8d2C7031d593D78b11119D0f3e72#code) via `attestCompliance(...)`. Anyone can call `verifyCompliance(report)` and get `MATCH`, proving the compliance decision shown in the UI is exactly what the agent committed on-chain — the compliance layer is auditable, not self-reported.
- **Auditability for regulators.** Because every agent decision *and its compliance verdict* are written on-chain (`recordDecision` + `attestCompliance`), there is a permanent, timestamped, tamper-evident audit trail of *what* the agent did, *why*, and *that it passed the eligibility gate* — useful for the suitability/record-keeping obligations that apply to managed-account products.
...
- **Attests its compliance verdict on-chain** via `attestCompliance(...)` on the [`StockPilotComplianceAttestor`](https://mantlescan.xyz/address/0x6d8aADb868CF8d2C7031d593D78b11119D0f3e72#code): the same verifiability applied to the AI rationale is applied to the pre-trade eligibility gate, so `verifyCompliance(report)` → `MATCH` for the exact verdict shown in the UI.
